...
import { fetchPaymentMethodTotalsForDay } from "../redux/slices/seminarSlice"; // Adjust path

// ProgressBar component with total amount display
const ProgressBar = ({ title, totalAmount, percentage, color }) => {
  // Format total amount as USD
...
const TopPaymentMethod = () => {
  ...
  const {
    paymentMethodTotals,
    paymentMethodTotalsStatus,
    paymentMethodTotalsError,
  } = useSelector((state) => state.seminar);

  // Fetch payment method totals on mount
  useEffect(() => {
    dispatch(fetchPaymentMethodTotalsForDay());
  }, [dispatch]);

  // Debugging logs
  console.log("paymentMethodTotals:", paymentMethodTotals);
  console.log("paymentMethodTotalsStatus:", paymentMethodTotalsStatus);
  console.log("paymentMethodTotalsError:", paymentMethodTotalsError);

  // Prepare progress bar data from paymentMethodTotals
  const paymentMethodData = () => {
    if (
      paymentMethodTotalsStatus !== "succeeded" ||
      !Array.isArray(paymentMethodTotals) ||
    ...
    const maxAmount = Math.max(
      ...paymentMethodTotals.map((m) => m.totalAmount || 0)
    );
    return paymentMethodTotals.map((method, index) => {
      const percentage =
  ...
  return (
    ...
    >
      ...
      {paymentMethodTotalsStatus === "loading" ? (
        <Box
          display="flex"
      ...
      ) : paymentMethodTotalsStatus === "failed" ? (
        <Typography fontSize={16} color="error.main" my="20px">
          Error: {paymentMethodTotalsError?.message || "Failed to load data"}
        </Typography>
      ) : (
