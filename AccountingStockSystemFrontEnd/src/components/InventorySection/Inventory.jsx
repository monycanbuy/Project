...
const Inventory = () => {
        ...
        return {
          ...
          batchExpiryDates:
            inventory.batches
              ...
              .join(", ") || "N/A",
          imageUrl: inventory.imageUrl || "N/A",
          reorderLevel:
  ...
  const handleEditClick = (inventory) => {
    if (!inventory || Object.keys(inventory).length < 21) {
      console.error("Invalid inventory data:", inventory);
      return;
    }
    const categoryObject = categories.find(
      (cat) => cat.name === inventory.categoryName
    );
    const supplierObject = suppliers.find(
      (sup) => sup.contactPerson === inventory.supplierName
    );
    const locationObject = locations.find(
      (loc) => loc.name === inventory.locationName
    );
    const inventoryData = {
      _id: inventory.id,
      name: inventory.name,
      description: inventory.description,
      costPrice: inventory.costPrice.replace(/[^0-9.-]+/g, ""),
      sellingPrice: inventory.sellingPrice.replace(/[^0-9.-]+/g, ""),
      category: categoryObject
        ? { _id: categoryObject._id, name: categoryObject.name }
        : { _id: "", name: "" },
      stockQuantity: inventory.stockQuantity,
      supplier: supplierObject ? supplierObject._id : "",
      batches: inventory.batchNumbers.split(", ").map((batchNumber, index) => ({
        batchNumber,
        quantity: parseInt(inventory.batchQuantities.split(", ")[index], 10),
        expiryDate: inventory.batchExpiryDates.split(", ")[index],
      })),
      ...
      locationName: locationObject ? locationObject._id : "",
      unit: inventory.unit,
      isPerishable: inventory.isPerishable === "Yes",
      lastRestocked: inventory.lastRestocked,
      isActive: inventory.isActive === "Active",
    };
    setEditData(inventoryData);
    setDrawerOpen(true);
  };
  ...
  const handleDeleteClick = (inventory) => {
    const inventoryId = inventory.id; // Extract the _id from the object
    setInventoryToDelete(inventoryId);
    setDeleteDialogOpen(true);
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      flex: 1,
      renderCell: (params) => {
        const isDanger = params.row.stockQuantity <= params.value;
        return (
          <StyledTableCell isDanger={isDanger}>{params.value}</StyledTableCell>
        );
      },
    },
    ...
    { field: "stockKeeperName", headerName: "Stock Keeper", flex: 1 },
