...
const AddNewSeminarDrawer = ({
  ...
  const handleSave = async () => {
    // console.log("handleSave triggered, editMode:", editMode); // Debug log
    ...
    try {
      if (editMode) {
        await dispatch(
          updateSeminar({ id: initialData._id, seminarData })
        ).unwrap();
        toast.success("Seminar record updated successfully!", {
          duration: 5000,
        });
      } else {
        await dispatch(createSeminar(seminarData)).unwrap();
        toast.success("Seminar record added successfully!", { duration: 5000 });
      }
      await dispatch(fetchSeminars()).unwrap(); // Refresh list
      onClose();
        ...
        <Box sx={{ p: 2, width: "100%", height: "100%", overflow: "auto" }}>
          ...
          <FormControl fullWidth margin="normal">
            ...
            >
              ...
              <MenuItem value="Outdoor Catering">Outdoor Catering</MenuItem>
            </Select>
          </FormControl>
          <FormControl fullWidth margin="normal">
